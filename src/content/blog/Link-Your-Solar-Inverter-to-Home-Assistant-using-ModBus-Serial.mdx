...
imageItem:
  image: ../../assets/esphome-solplanet-modbus-rs485-home-assistant.webp
...
* **[ESP8266](https://amzn.to/4m7IJDq)** or **[ESP32](https://amzn.to/3YvYM3P)**: Low-cost Wi-Fi microchips with built-in TCP/IP protocol stack.
...
* **[RJ45](https://amzn.to/44XHTmq)**: A type of connector commonly used for Ethernet networking.
* **T-568B**: A very common wiring standard for terminating Ethernet cables, specifying the pinout for the RJ45 connector.

Some of the product links in this article are affiliate links. I added them afterward to the mentioned items. Using these links helps support my work on articles like this one—thank you if you choose to use them! If not, no worries at all. 😊
...
* **[ESP8266](https://amzn.to/4m7IJDq) or [ESP32](https://amzn.to/3YvYM3P)**
* **RS485 board to use with ESP8266 (recommended: [MAX485 TTL](https://amzn.to/4ddu4CE))**
* **A spare [ethernet cable ](https://amzn.to/4d8oOQC)for the RJ45.**

### Debugging

**Before proceeding**, verify that you can get MODBUS serial to work with your inverter. I used a [USB RS485 board](https://amzn.to/44kheA3) for easy debugging with a trial of [Modbus Poll](https://www.modbustools.com/download.html "Modbus Poll"). I have wasted a lot of time starting directly on the [ESP8266](https://amzn.to/4m7IJDq) instead of starting with some isolated testing.

### Hardware Setup

For this solution, I used an [ESP8266](https://amzn.to/4m7IJDq) with a [MAX485](https://amzn.to/4ddu4CE). Alternatively, you can use a UART TTL to RS485 board with automatic flow control, although I found the more reliable. Setting up flow control on an ESPHome installation is just setting one configuration value.
...
* **[ESP8266](https://amzn.to/4m7IJDq) to [MAX485](https://amzn.to/4ddu4CE)**
  * 3V or VIN to VCC (I tried 3V but had to switch to VIN, be **mindful** that the USB on the inverter is NOT 5V)
  * GND to GND
  * TX to DI
  * RX to RO
  * D5 to DE & RE
